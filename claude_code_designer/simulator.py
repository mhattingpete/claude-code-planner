...
class DesignSimulator:
    ...
    def __init__(
        ...
        self.evaluator = ConversationEvaluator(str(self.conversation_dir), str(self.evaluation_dir))
        self.scenario_generator = ScenarioGenerator()
        
        if enable_learning:
            self.learning_system = LearningSystem(
                str(self.learning_knowledge_dir),
                str(self.evaluation_dir)
            )
        else:
            self.learning_system = None
    
    async def run_single_cycle(self, scenario: TestScenario | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if scenario.scenario_type == "app":
                design_result = await self.design_assistant.design_application(**scenario.parameters)
            else:
                design_result = await self.design_assistant.design_feature(**scenario.parameters)
            ...
            if conversation_file.exists():
                evaluation_result = await self.evaluator.evaluate_conversation(conversation_file)
                ...
                if self.enable_learning and self.learning_system:
                    ...
                    eval_file = Path(str(conversation_file).replace("conversations", "evaluations").replace(".json", "_evaluation.json"))
                    if eval_file.exists():
                        await self.learning_system.learn_from_evaluations([eval_file])
                        
                        # Show learning stats
                        stats = self.learning_system.get_knowledge_stats()
                        print(f"    📚 Knowledge base now has {stats.get('total_rules', 0)} rules")
    ...
    async def run_simulation_loop(
        ...
        for cycle_num in range(1, max_cycles + 1):
            ...
            await self._save_simulation_results(all_results, f"simulation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    ...
    async def _save_simulation_results(self, results: list[dict[str, Any]], filename: str):
    ...
    def _generate_summary_stats(self, results: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_design_messages": sum(
                ...
                if r["design_session"]
            ) / len(successful_cycles) if successful_cycles else 0
        }
    ...
    def _print_simulation_summary(self, results: list[dict[str, Any]]):
        ...
        print(f"   Feature scenarios: {stats['scenario_breakdown']['feature_scenarios']}")
