...
class Question(BaseModel):
    ...
    options: list[str] | None = Field(None, description="Available options for multiple choice questions")
    required: bool = Field(True, description="Whether this question is required")
    follow_up: dict[str, str] | None = Field(None, description="Follow-up questions based on answers")
...
class AppDesign(BaseModel):
    ...
    primary_features: list[str] = Field(default_factory=list, description="List of primary features")
    tech_stack: list[str] = Field(default_factory=list, description="Technology stack preferences")
    ...
    additional_info: dict[str, Any] = Field(default_factory=dict, description="Additional design information")


class DocumentRequest(BaseModel):
    """Model for document generation configuration."""

    output_dir: str = Field(..., description="Directory where documents should be generated")
    ...
    app_design: AppDesign = Field(..., description="Application design data to use for generation")
    custom_templates: dict[str, str] | None = Field(None, description="Custom templates for documents")
